...
?>
				...
				<form id="membershipform" align="left" action="http://13.125.107.155/membership_rewelcome.php?login=yes" method="post">
					...
					if(mysqli_connect_errno($con)){
						...
						$db_check1 = "SELECT * FROM login WHERE email = '$_emailInfo'";
						$db_check_rst1 = mysqli_query($con, $db_check1);
							$row = mysqli_fetch_array($db_check_rst1);
							$_nickname = $row['nickname'];
							//아래 echo 부분을 수정해야합니다.  nickname 가 나타날수 있도록
							// echo "<script>alert(\"$_emailInfo\");</script>";
								// echo "<script>alert(\"$_nickname\");</script>";

					}
					?>
					<input type="hidden" name="emailInfo" value="<?php echo $_emailInfo ?>">
						<input type="hidden" name="nickname" value="<?php echo $_nickname ?>">
