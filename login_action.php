...

{
	...
	if(p1.value){
...

		{
			...
			window.open(ref, "가입 확인 진행", 'width=250, height=160, status=no, top='+window_top+', left='+window_left+'');
...
<?php
	...
	$con = mysqli_connect("127.0.0.1", "각자의 데이터베이스 아이디", "각자 설정한 자신의 데이터베이스 비밀번호", "joelstudydb");
	...
	if(mysqli_connect_errno($con)){
		...
		if( ("" != $row['email']) && ($_password == $row['password'])){
...

			{
				...
				echo "가입 확인을 위해 전자 메일로 받으신 인증 코드를 입력해 주세요 <br>";
