<?php
  ...
  $con = mysqli_connect("127.0.0.1", "root", "a1214511", "joeltestdb");
   $sql = "INSERT INTO shop(b_no, b_title, b_contents, b_date, b_hit, b_image, nickname , video ,b_price)
   values(null, '{$bTitle}','{$bContent}','{$date}',0,'{$bImage}','{$bID}','{$video}','{$bPrice}' );";
  $queryresult = mysqli_query($con, $sql);
	if($queryresult) { // query가 정상실행 되었다면,
		$msg = "글이 등록되었습니다.";
    $bNo = $con->insert_id;
	$replaceURL = './post.php?bid='. $bID .'?bno='. $bNo;
	}
  else {
		$msg = "글을 등록하지 못했습니다.";
?>
