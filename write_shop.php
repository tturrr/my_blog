<?php
  ...
  //그래서  구분자로 나누어서 받는다.
		...
		if(isset($bno)) {
		 	...
		 	$sql = "SELECT b_title, b_contents,b_image,nickname,video,b_price from shop where b_no = ". $bno;
...
<script>
	...
	function submitBack(){
		location.href='post.php?<?php echo $_SESSION['testuser']?>';
	}
    	...
    	<div class="banner-dott">
    		<div class="main" >
    				...
    				<h1 class="w3layouts_head">글 작성</h1>
						<form enctype="multipart/form-data" name="nse" action="<?php echo isset($bno)?'write_modify_shop.php':'write_update_shop.php'?>			" method="post">
							<?php
								if(isset($bno)) {?>
                ...
                <input type="number" name="b_price" placeholder="0원" value="<?php echo isset($row['b_price'])?$row['b_price']:null?>" />
